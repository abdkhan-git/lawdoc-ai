...
import { LogIn, FileText, ArrowRight, Loader2 } from "lucide-react";
...
export default function HomeClient({ isAuth }: HomeClientProps) {
  ...
  // Handle navigation to most recent chat or chat list
  const handleGoToChats = async () => {
    ...
    try {
      ...
      if (chatId) {
        // If found a chat, navigate to it
        router.push(`/chat/${chatId}`);
      } else {
        // No chats found, redirect to main page
        router.push("/");
      }
    } catch (error) {
      console.error("Error fetching chat:", error);
      // On error, just go to main page
      router.push("/");
    } finally {
      setLoading(false);
    }
          ...
          <div className="py-6 md:py-8">
            {isAuth ? (
              <div className="space-y-6">
                {/* Replace Link with Button that uses handleGoToChats */}
                <Button 
                  onClick={handleGoToChats} 
                  disabled={loading}
                  size="lg" 
                  className="bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-6 md:px-8 py-3 md:py-4 text-base md:text-lg rounded-xl shadow-lg shadow-blue-900/50 transition-all hover:shadow-xl"
                >
                  {loading ? (
                    <>
